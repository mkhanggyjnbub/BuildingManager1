...
<html>
    <head>
        ...
        <style>
            /* (Toàn bộ phần style giữ nguyên như phiên bản trước đó - đã đẹp và chuẩn) */
            :root {
                --primary: #002b5c;
            ...
            .dot-icon:hover { color: var(--primary); }
            ...
            .dropdown a:hover { background-color: #eef4ff; }
            ...
            .pagination li a:hover { background-color: var(--primary-light); }
    ...
    <body>
                    ...
                    <c:forEach var="user" items="${list}" varStatus="i">
                        <tr>
                                ...
                                <div class="dropdown">
                                    ...
                                    <c:choose>
                                        <c:when test="${user.status eq 'Active'}">
                                            <a href="#" onclick="confirmDelete(${user.userId}, '${user.status}')">
                                                <i class="fa-solid fa-user-slash"></i> Delete
                                            </a>
                                        </c:when>
                                        <c:otherwise>
                                            <a href="#" onclick="confirmDelete(${user.userId}, '${user.status}')">
                                                <i class="fa-solid fa-user-check"></i> Restore
                                            </a>
                                        </c:otherwise>
                                    </c:choose>
                            ...
                            <td class="actions">
                                ...
                                <c:choose>
                                    <c:when test="${user.status eq 'Active'}">
                                        <a href="#" onclick="confirmDelete(${user.userId}, '${user.status}')" title="Delete">
                                            <i class="fa-solid fa-user-slash"></i>
                                        </a>
                                    </c:when>
                                    <c:otherwise>
                                        <a href="#" onclick="confirmDelete(${user.userId}, '${user.status}')" title="Restore">
        ...
        <script>
            function confirmDelete(userId, status) {
                let isDeleting = status === "Active";
                Swal.fire({
                    title: isDeleting ? 'Confirm deletion?' : 'Recover account?',
                    text: isDeleting
                        ? "Are you sure you want to disable this account?"
                        : "Do you want to recover this account?",
                    icon: isDeleting ? 'warning' : 'question',
                    showCancelButton: true,
                    confirmButtonColor: isDeleting ? '#d33' : '#3085d6',
                    cancelButtonColor: '#999',
                    confirmButtonText: isDeleting ? 'Delete' : 'Restore',
                    cancelButtonText: 'Cancel'
                }).then((result) => {
                    if (result.isConfirmed) {
                        window.location.href = 'DeleteStaff?id=' + userId;
                    }
                });
            }
        </script>
