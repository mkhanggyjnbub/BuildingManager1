...
public class ResetPassword extends HttpServlet {
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response)
        ...
        if (username == null || email == null) {
            response.sendRedirect("Login"); // hết phiên hoặc truy cập sai luồng
            return;
        }
        ...
        if (newPassword.length() < 8 || newPassword.length() > 100) {
            request.setAttribute("error", "Mật khẩu phải từ 8 đến 100 ký tự.");
            request.getRequestDispatcher("account/resetPassword.jsp").forward(request, response);
            return;
        }

        CustomerDao dao = new CustomerDao();
        String hashedPassword = dao.md5(newPassword);

        boolean success = dao.updatePassword(username, email, hashedPassword);
