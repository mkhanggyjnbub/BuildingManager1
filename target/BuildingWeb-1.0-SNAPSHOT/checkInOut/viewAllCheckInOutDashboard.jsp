...
<head>
    ...
    <script>
        function confirmAction(form, type) {
            ...
            return confirm(message); // Trả về true mới submit
        }
    </script>
</head>
<body>
        ...
        <c:forEach var="booking" items="${checkInList}">
            <c:if test="${booking.status != 'Waiting' && booking.status != 'Waiting for processing'}">
                <tr>
                    ...
                    <td data-label="Room">
                        <c:choose>
                            <c:when test="${booking.rooms != null}">${booking.rooms.roomNumber}</c:when>
                            <c:otherwise>N/A</c:otherwise>
                        </c:choose>
                    </td>
                    <td data-label="Status">
                        <c:choose>
                            <c:when test="${booking.status == 'Confirmed'}">
                                <span class="status waiting"><i class="fas fa-clock btn-icon"></i>Waiting</span>
                            </c:when>
                            <c:when test="${booking.status == 'Checked in'}">
                                <span class="status staying"><i class="fas fa-door-open btn-icon"></i>Staying</span>
                            </c:when>
                        ...
                        <c:choose>
                            <c:when test="${booking.status == 'Confirmed'}">
                                <form action="ViewAllCheckInOutDashboard" method="post" style="display:inline;"
                                      onsubmit="return confirmAction(this, 'CheckIn');">
                                    <input type="hidden" name="bookingId" value="${booking.bookingId}"/>
                                    <input type="hidden" name="action" value="Checked in"/>
                                    <button class="btn-action btn-checkin"><i class="fas fa-sign-in-alt btn-icon"></i>Check-In</button>
                                </form>
                            </c:when>
                            <c:when test="${booking.status == 'Checked in'}">
                                <form action="ViewAllCheckInOutDashboard" method="post" style="display:inline;"
                            ...
                            <c:when test="${booking.status == 'Checked out'}">
                                <form action="ViewAllCheckInOutDashboard" method="post" style="display:inline;"
                                      onsubmit="return confirmAction(this, 'CheckOut');">
                                   <i class="fas fa-sign-out-alt btn-icon">Checked-Out</i>
                                </form>
                            </c:when>
